...
const ManageUsers = () => {
    ...
    const {data: users = [], refetch} = useQuery(['users'], async() => {
        const res =  await fetch('https://summer-camp-school-server-side-pi.vercel.app/users')
        return res.json()
    })

  
const handleMakeAdmin = user =>{
   fetch(`https://summer-camp-school-server-side-pi.vercel.app/users/admin/${user._id}`,{
    method: 'PATCH'
})
.then(res => res.json())
.then(data => {
...
const handleMakeInstructor = user =>{
    fetch(`https://summer-camp-school-server-side-pi.vercel.app/users/instructor/${user._id}`,{
     method: 'PATCH'
 })
 .then(res => res.json())
 .then(data => {
